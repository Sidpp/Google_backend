...
if (!OPENAI_API_KEY || !MONGO_URI || !DB_NAME || !PROCESSED_DATA_COLLECTION) {
...
// --- AI Prompt ---
// This remains identical to the Python version
...
const processSingleRecord = async (record) => {
  try {
    ...
    const { spreadsheet_id, row_index, project_identifier, sync_timestamp, input_data } = sqsPayload;
    logger.info(`Processing: ${project_identifier} (Row ${row_index}) from SQS source: ${record.eventSourceARN}`);
    ...
    const documentToStore = MongoDbSchema.parse({
      spreadsheet_id,
      row_index,
      project_identifier,
      sync_timestamp,
    ...
    const upsertKey = { spreadsheet_id, row_index };
    const success = await storeDocumentWithRetry(documentToStore, upsertKey);
    ...
    logger.error(`Failed to process record ${record.messageId}: ${error.message}`, { stack: error.stack });
