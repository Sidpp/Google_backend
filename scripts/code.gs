...
function onSheetEdit(e) {
  try {
    ...
    const numRows = range.getNumRows(); // Get the number of rows that were edited.

    // Get all headers once.
    const headers = sheet.getRange(1, 1, 1, sheet.getLastColumn()).getValues()[0];
    // Get all the data for all edited rows in a single call for efficiency.
    const editedRowsData = sheet.getRange(startRow, 1, numRows, sheet.getLastColumn()).getValues();

    // Loop through each row that was part of the edit.
    for (let i = 0; i < numRows; i++) {
      ...
      // Send a separate update for each row.
